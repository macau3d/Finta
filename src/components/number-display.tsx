...
export function HitBadge({ value, taruhanSet, size = "sm" }: HitBadgeProps) {
  ...
  const colorCls =
    hits === 0 ? "bg-slate-500/10 dark:bg-slate-700/50 text-slate-500 border-slate-400/20 dark:border-slate-600/40" :
  ...
  return (
    <span className={cn("inline-flex flex-col items-center gap-2 font-mono", className)}>
      <span className="inline-flex items-baseline gap-1">
        <span className="text-4xl sm:text-5xl font-black tabular-nums leading-none tracking-tight text-foreground">{front}</span>
        <span className="text-base sm:text-lg font-bold tabular-nums leading-none text-slate-500 dark:text-slate-400">{back}</span>
      </span>
      {showTags && cls && (
