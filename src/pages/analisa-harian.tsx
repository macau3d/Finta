...
function makeDual(
  ...
  const aPctRaw = total > 0 ? (aCount / total) * 100 : 50;
  const bPctRaw = total > 0 ? 100 - aPctRaw : 50;
  const winner = aPctRaw > bPctRaw ? "a" : bPctRaw > aPctRaw ? "b" : "tie";
  const aPct = winner === "a" ? 99 : winner === "b" ? 1 : 50;
  const bPct = winner === "b" ? 99 : winner === "a" ? 1 : 50;
  const confidence = 99;
  return {
...
function SlotCard({ analysis, activeMarket, defaultOpen }: { analysis: SlotAnalysis; activeMarket: MarketType; defaultOpen: boolean; key?: any }) {
  ...
  return (
    <div className={cn(
      "overflow-hidden rounded-3xl border transition-all shadow-sm backdrop-blur-sm",
  ...
  return (
    <div className="mx-auto max-w-5xl space-y-5 px-4 pb-24 pt-6 md:pb-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-4">
        <div>
          <div className="flex items-center gap-2">
            ...
            <h1 className="text-2xl font-bold text-foreground font-display tracking-tight">Analisa Harian</h1>
          </div>
        ...
        <div className="flex flex-wrap items-center gap-2 glass-card p-1.5 rounded-2xl shrink-0">
      ...
      {todaySummary && todaySummary.length > 0 && (
        <div className="rounded-[2rem] glass-card p-5 border-primary/20 bg-primary/5">
          <div className="mb-3 text-[10px] font-bold uppercase tracking-wider text-primary">Hasil Hari Ini</div>
          <div className="flex flex-wrap gap-2">
            {todaySummary.map((s) => s && (
              <div key={s.slot} className="flex items-center gap-3 rounded-xl border border-black/10 dark:border-white/10 bg-black/5 dark:bg-white/5 px-4 py-2.5 shadow-sm">
                ...
                <div className="flex gap-1.5">
                  <span className={cn("rounded-md px-1.5 py-0.5 text-[9px] font-bold", s.ganjil ? "bg-amber-500/20 text-amber-300" : "bg-sky-500/20 text-sky-300")}>
                    {s.ganjil ? "G" : "A"}
                  </span>
                  <span className={cn("rounded-md px-1.5 py-0.5 text-[9px] font-bold", s.besar ? "bg-rose-500/20 text-rose-300" : "bg-green-500/20 text-green-300")}>
                    {s.besar ? "B" : "K"}
                  </span>
                  <span className={cn("rounded-md px-1.5 py-0.5 text-[9px] font-bold", s.ekorGanjil ? "bg-violet-500/20 text-violet-300" : "bg-teal-500/20 text-teal-300")}>
                    E{s.ekorGanjil ? "G" : "A"}
                  </span>
                  <span className={cn("rounded-md px-1.5 py-0.5 text-[9px] font-bold", s.ekorBesar ? "bg-orange-500/20 text-orange-300" : "bg-cyan-500/20 text-cyan-300")}>
                    E{s.ekorBesar ? "B" : "K"}
                  </span>
                </div>
              </div>
          ...
          <div className="mt-4 flex flex-wrap gap-4 text-[10px] text-muted-foreground/70 bg-black/5 dark:bg-white/5 p-2 rounded-lg inline-flex">
            <span><strong className="text-amber-500 dark:text-amber-300">G</strong>=Ganjil <strong className="text-sky-500 dark:text-sky-300">A</strong>=Genap</span>
            <span><strong className="text-rose-500 dark:text-rose-300">B</strong>=Besar <strong className="text-green-500 dark:text-green-300">K</strong>=Kecil</span>
            <span><strong className="text-violet-500 dark:text-violet-300">EG</strong>=Ekor Ganjil <strong className="text-teal-500 dark:text-teal-300">EA</strong>=Ekor Genap</span>
            <span><strong className="text-orange-500 dark:text-orange-300">EB</strong>=Ekor Besar <strong className="text-cyan-500 dark:text-cyan-300">EK</strong>=Ekor Kecil</span>
          </div>
        </div>
      )}
      ...
      {/* Guide */}
      <div className="rounded-3xl border border-border/50 bg-muted/20 p-6 text-xs text-muted-foreground space-y-3 shadow-sm backdrop-blur-sm">
