...
function StatCard({ label, value, sub, color }: { label: string; value: string | number; sub?: string; color?: string }) {
  return (
    <div className="rounded-[2rem] glass-card p-6 text-center shadow-sm">
      <div className={cn("text-3xl sm:text-4xl font-black font-display tracking-tight", color ?? "text-foreground drop-shadow-[0_0_8px_rgba(255,255,255,0.3)]")}>{value}</div>
      <div className="text-[10px] font-bold text-muted-foreground uppercase tracking-wider mt-3">{label}</div>
      {sub && <div className="text-[9px] font-medium text-muted-foreground mt-1">{sub}</div>}
    </div>
  );
}
    ...
    <div className="mx-auto max-w-6xl space-y-5 px-4 pb-24 pt-6 md:pb-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div className="flex items-center gap-2">
          ...
          <h1 className="text-2xl font-bold text-foreground font-display tracking-tight">Riwayat Prediksi</h1>
        ...
        <div className="flex items-center gap-3 flex-wrap">
              ...
              <button
                ...
                className={cn(
                  ...
                  activeMarket === key
                    ...
                    : "bg-transparent text-muted-foreground hover:bg-black/5 dark:bg-white/5 hover:text-foreground"
                )}
              >
                {name}
              </button>
          ...
          <button 
            ...
            className="flex items-center gap-2 rounded-xl bg-primary px-4 py-2 text-xs font-bold text-primary-foreground hover:bg-primary/90 disabled:opacity-50 disabled:cursor-not-allowed transition-colors shadow-[0_0_15px_rgba(var(--primary),0.3)]"
          >
      ...
      {isLoading ? (
        ...
        <div className="rounded-[2rem] glass-card p-12 text-center shadow-sm">
          <div className="flex h-16 w-16 mx-auto items-center justify-center rounded-2xl bg-black/5 dark:bg-white/5 border border-black/10 dark:border-white/10 mb-5">
            <History className="h-8 w-8 text-muted-foreground/50" />
          </div>
          <div className="text-lg font-bold text-foreground mb-2">Belum ada prediksi tersimpan</div>
          <div className="text-sm text-muted-foreground max-w-[300px] mx-auto">
        ...
        <>
          ...
          {/* Per-position accuracy */}
          {totalResolved > 0 && (
            <div className="rounded-[2rem] glass-card p-6 space-y-4 shadow-sm">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
                {posAcc.map((p, i) => (
                  <div key={i} className="text-center rounded-2xl border border-black/5 dark:border-white/5 bg-white/[0.02] p-4 transition-colors hover:bg-black/5 dark:bg-white/5">
                    <div className="text-[10px] text-muted-foreground font-bold uppercase tracking-widest">{POS_LABELS[i]}</div>
                    <div className={cn("text-3xl font-black font-mono mt-2 mb-1",
                      p.pct >= 70 ? "text-green-400 drop-shadow-[0_0_8px_rgba(34,197,94,0.4)]" : p.pct >= 50 ? "text-amber-400 drop-shadow-[0_0_8px_rgba(251,191,36,0.4)]" : p.pct >= 30 ? "text-orange-400 drop-shadow-[0_0_8px_rgba(249,115,22,0.4)]" : "text-red-400 drop-shadow-[0_0_8px_rgba(239,68,68,0.4)]"
                    )}>{p.pct}%</div>
                    <div className="text-[10px] font-medium text-muted-foreground">{p.correct} benar dari {p.eligible}</div>
                    <div className="mt-3 h-1.5 rounded-full bg-black/10 dark:bg-white/10 overflow-hidden shadow-inner">
                      <div
                        className={cn("h-full rounded-full transition-all duration-1000", p.pct >= 70 ? "bg-green-500" : p.pct >= 50 ? "bg-amber-500" : p.pct >= 30 ? "bg-orange-500" : "bg-red-500")}
                        style={{ width: `${p.pct}%` }}
                      />
                    </div>
                  </div>
              ...
              <div className="h-[220px] w-full">
                <ResponsiveContainer width="100%" height="100%">
                  <AreaChart data={chartData}>
                    ...
                    <Tooltip 
                      contentStyle={{ backgroundColor: 'rgba(0,0,0,0.8)', border: '1px solid rgba(255,255,255,0.1)', borderRadius: '12px', fontSize: '12px', backdropFilter: 'blur(8px)' }}
                      itemStyle={{ color: '#00dcff', fontWeight: 'bold' }}
                    />
                    <Area type="monotone" dataKey="score" stroke="#00dcff" strokeWidth={3} fillOpacity={1} fill="url(#colorScore)" activeDot={{ r: 6, fill: '#00dcff', stroke: '#fff', strokeWidth: 2 }} />
                  </AreaChart>
                </ResponsiveContainer>
              </div>
            </div>
                ...
                <tbody className="divide-y divide-black/10 dark:divide-white/10">
                  {filtered.map((p) => {
                    ...
                    return (
                      ...
                      <td className="px-3 py-4 font-mono text-xs text-foreground font-medium">{p.forDate}</td>
                      <td className="px-3 py-4">
                      ...
                      <td className="px-3 py-4 text-center">
                        <div className="flex justify-center gap-2">
                          {parseBbfs(p.bbfs).map((digits, i) => (
                            <div key={i} className="text-center rounded-xl bg-black/40 border border-black/5 dark:border-white/5 px-1.5 py-1">
                              <div className="text-[8px] text-muted-foreground uppercase font-bold tracking-widest mb-1">{POS_LABELS[i]}</div>
                                ...
                                {digits.map((d, j) => {
                                  ...
                                  return (
                                    <span key={j} className={cn(
                                      "flex h-5 w-4 items-center justify-center rounded-[4px] font-mono text-[10px] font-black border",
                                      isMatched ? "bg-green-500/20 text-green-300 border-green-500/40" : "bg-black/5 dark:bg-white/5 text-muted-foreground/80 border-black/10 dark:border-white/10"
                                    )}>
                                      {d}
                                    </span>
                                  );
                      ...
                      <td className="px-3 py-4 text-center font-mono text-xs font-bold">
                      ...
                      <td className="px-3 py-4 text-center">
                        {p.signal === "yes" || p.signal === "STRONG_BUY" || p.signal === "STRONG" || p.signal === "strong"
                          ? <CheckCircle2 className="inline h-5 w-5 text-green-400 drop-shadow-[0_0_5px_rgba(34,197,94,0.5)]" />
                          : <XCircle className="inline h-5 w-5 text-red-400/50" />
                        }
                      </td>
                      <td className="px-5 py-4 text-center">
                        <button
                          ...
                          className="rounded-xl p-2 text-muted-foreground/40 hover:bg-red-500/10 hover:text-red-400 transition-colors disabled:opacity-50"
                        >
          ...
          <div className="space-y-4 md:hidden">
            {filtered.map((p) => {
              const actualDigits = p.actual4d ? p.actual4d.split("") : null;
              return (
                <div key={p.id} className={cn("rounded-[2rem] glass-card p-6 space-y-5 shadow-sm", p.digitScore === 4 ? "border-green-500/30 shadow-[0_0_15px_rgba(34,197,94,0.1)]" : "")}>
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-3 flex-wrap">
                      <span className={cn("rounded-lg px-2 py-1 font-mono text-[10px] font-bold", SESSION_COLORS[p.session] ?? "bg-black/10 dark:bg-white/10 text-muted-foreground")}>
                        {MARKET_SESSION_LABELS[activeMarket][p.session] ?? p.session}
                      </span>
                      <span className="font-mono text-xs font-medium text-foreground">{p.forDate}</span>
                    </div>
                    <button
                      onClick={() => handleDelete(p.id)}
                      disabled={deleting === p.id}
                  ...
                  <div className="flex items-center justify-between">
                    <div>
                      <div className="text-[10px] font-bold tracking-widest text-muted-foreground uppercase mb-2">Prediksi / Aktual</div>
                      <div className="flex gap-4">
                        {Array.from({ length: 4 }).map((_, i) => {
                          const predChar = p.predicted4d?.[i] ?? "-";
                          return (
                            <div key={i} className="text-center">
                              <div className="text-[9px] font-bold tracking-widest text-muted-foreground mb-1">{POS_LABELS[i]}</div>
                              <DigitCell
                                predicted={predChar}
                                actual={actualDigits ? actualDigits[i] ?? null : null}
                                correct={p[POS_KEYS[i]!]}
                    ...
                    <div className="text-right space-y-2">
                      ...
                      <div className="flex items-center justify-end gap-1.5">
                        <span className={cn("font-mono text-sm font-black", p.confidence >= 60 ? "text-green-400" : "text-muted-foreground")}>{p.confidence}%</span>
                        {p.signal === "yes" || p.signal === "STRONG_BUY" || p.signal === "STRONG" || p.signal === "strong"
                          ? <CheckCircle2 className="h-4 w-4 text-green-400 drop-shadow-[0_0_5px_rgba(34,197,94,0.5)]" />
                  ...
                  <div className="border-t border-black/10 dark:border-white/10 pt-4">
                    <div className="text-[10px] text-muted-foreground uppercase mb-2 font-bold tracking-wider">BBFS 5 Digit per Posisi</div>
                    <div className="flex gap-3 flex-wrap">
                      {parseBbfs(p.bbfs).map((digits, i) => (
                        <div key={i} className="text-center rounded-xl bg-black/40 border border-black/5 dark:border-white/5 px-2 py-1.5">
                          <div className="text-[9px] text-muted-foreground uppercase font-bold mb-1.5">{POS_LABELS[i]}</div>
                          <div className="flex gap-1">
                            {digits.map((d, j) => {
                              ...
                              return (
                                <span key={j} className={cn(
                                  "flex h-6 w-5 items-center justify-center rounded-md font-mono text-xs font-black border",
                                  isMatched ? "bg-green-500/25 text-green-300 border-green-500/40" : "bg-black/5 dark:bg-white/5 text-muted-foreground/80 border-black/10 dark:border-white/10"
                                )}>
                                  {d}
                                </span>
                              );
                  ...
                  {p.actual4d === null && (
                    <div className="flex items-center gap-2 rounded-xl border border-dashed border-amber-500/30 bg-amber-500/10 px-3 py-2.5">
                      <AlertCircle className="h-4 w-4 text-amber-400/80 shrink-0" />
                      <span className="text-[10px] font-medium text-amber-300/80">Hasil aktual belum tersedia — otomatis terisi saat data draw masuk.</span>
                    </div>
                  )}
                </div>
              );
